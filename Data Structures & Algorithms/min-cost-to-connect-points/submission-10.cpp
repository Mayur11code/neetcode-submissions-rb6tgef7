...
priority_queue<pair<int, vector<int>>, vector<pair<int, vector<int>>>, greater<pair<int, vector<int>>>> pq;
        ...
        while(!pq.empty() && nodes<points.size()){
            ...
            for(auto& point : points){
                if(hashed(point[0], point[1],points) != hashed(p.second[0], p.second[1],points) && !visited.contains(hashed(point[0],point[1],points))){
        ...
        return nodes <= points.size() ? cost : -1;
